...
import { CustomFontConstant, FontSize, Images, safePadding, StorageKey } from '@/constants/GeneralConstants';
...
const AuthScreen = () => {
  ...
  const { continueWithPhone, isLoading, error, showError, setShowError } = useAuth();
  ...
  // Pre-fill with whatever phone number was last used on this device, so a
  // returning user only has to tap Continue and enter the OTP.
  useEffect(() => {
    AsyncStorage.getItem(StorageKey.lastPhoneNumber).then((saved) => {
      if (saved) {
  ...
  const onSubmit: SubmitHandler<PhoneFormData> = (data) => {
    ...
    Keyboard.dismiss();
    AsyncStorage.setItem(StorageKey.lastPhoneNumber, data.phone);
    continueWithPhone(phone_number, formattedPhone);
  };

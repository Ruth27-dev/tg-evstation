import { fetchTopupAmount } from "@/services/useApi";
...
export const useAmount = () => {
    ...
    const { amountData, setAmountData } = useAmountStore();

    const getAmount = async (page: number = 1) => {
        try {
            if (page === 1) {
                setIsLoading(true);
            } else {
            ...
            const response = await fetchTopupAmount(data);
            if (response?.data?.code === '000') {
                ...
                if (page === 1) {
                    setAmountData(newContent);
                } else {
                    const currentTransactions = amountData || [];
                    ...
                    setAmountData([...currentTransactions, ...uniqueNewContent]);
                }
